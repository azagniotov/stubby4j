...
public class SslUtils {
    ...
    private static boolean isTLSv13SupportedByJDK0() {
        try {
            return arrayContains(DEFAULT_SSL_CONTEXT.getSupportedSSLParameters().getProtocols(), TLS_v1_3);
    ...
    private static boolean isTLSv13EnabledByJDK0() {
        try {
            return arrayContains(DEFAULT_SSL_CONTEXT.getDefaultSSLParameters().getProtocols(), TLS_v1_3);
        } catch (Throwable cause) {
            cause.printStackTrace();
            return false;
        }
