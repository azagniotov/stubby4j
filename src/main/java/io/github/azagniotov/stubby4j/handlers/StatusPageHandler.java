...
public final class StatusPageHandler extends AbstractHandler implements AbstractHandlerExtension {
    ...
    private static final List<ConfigurableYAMLProperty> FIELDS_FOR_AJAX_LINKS =
            unmodifiableList(asList(FILE, BODY, POST));

    private static final String TEMPLATE_LOADED_FILE_METADATA_PAIR =
            "<span style='color: #8B0000'>%s</span>=<span style='color: green'>%s</span>";
    private static final String TEMPLATE_AJAX_TO_RESOURCE_HYPERLINK =
            "<strong><a class='ajax-resource' href='/ajax/resource/%s/%s/%s'>[view]</a></strong>";
    private static final String TEMPLATE_AJAX_TO_STATS_HYPERLINK =
            "<strong><a class='ajax-stats' href='/ajax/stats'>[view]</a></strong>";
    private static final String TEMPLATE_HTML_TABLE_ROW =
            "<tr><td width='250px' valign='top' align='left'>%s</td><td align='left'>%s</td></tr>";
    ...
    public void handle(
        ...
        response.setHeader(
                HttpHeader.SERVER.asString().toLowerCase(Locale.US), HandlerUtils.constructHeaderServerName());
    ...
    private String buildStubProxyConfigtHtmlTable(final StubProxyConfig stubProxyConfig, final String templateHtmlTable)
            throws Exception {
        final String proxyUuid = stubProxyConfig.getUUID();
        final String ajaxLinkToRequestAsYaml =
                String.format(TEMPLATE_AJAX_TO_RESOURCE_HYPERLINK, PROXY_CONFIG, proxyUuid, "proxyConfigAsYAML");
        final StringBuilder proxyConfigTableBuilder = buildStubHtmlTableBody(
                proxyUuid, PROXY_CONFIG.toString(), ReflectionUtils.getProperties(stubProxyConfig));
    ...
    private String buildStubRequestHtmlTable(final StubHttpLifecycle stubHttpLifecycle, final String templateHtmlTable)
            throws Exception {
        final String resourceId = stubHttpLifecycle.getResourceId();
        final String ajaxLinkToRequestAsYaml = String.format(
                TEMPLATE_AJAX_TO_RESOURCE_HYPERLINK,
                resourceId,
                ConfigurableYAMLProperty.HTTPLIFECYCLE,
                "requestAsYAML");
        final StringBuilder requestTableBuilder = buildStubHtmlTableBody(
                resourceId, REQUEST.toString(), ReflectionUtils.getProperties(stubHttpLifecycle.getRequest()));
    ...
    private String buildStubResponseHtmlTable(final StubHttpLifecycle stubHttpLifecycle, final String templateHtmlTable)
            throws Exception {
        final String resourceId = stubHttpLifecycle.getResourceId();
        ...
        for (int sequenceId = 0; sequenceId < allResponses.size(); sequenceId++) {
            ...
            final String nextResponseLabel =
                    (isResponsesSequenced && nextSequencedResponseId == sequenceId ? NEXT_IN_THE_QUEUE : "");
            final String responseTableTitle = (isResponsesSequenced
                    ? String.format("%s/%s%s", RESPONSE, sequenceId, nextResponseLabel)
                    : RESPONSE.toString());
            ...
            final StringBuilder sequencedResponseBuilder =
                    buildStubHtmlTableBody(resourceId, responseTableTitle, stubResponseProperties);
            final String ajaxLinkToResponseAsYaml = String.format(
                    TEMPLATE_AJAX_TO_RESOURCE_HYPERLINK,
                    resourceId,
                    ConfigurableYAMLProperty.HTTPLIFECYCLE,
                    "responseAsYAML");
            ...
            responseTableBuilder.append(
                    String.format(templateHtmlTable, responseTableTitle, sequencedResponseBuilder.toString()));
        }
    ...
    private String buildJvmParametersHtmlTable(final String templateHtmlTable) throws Exception {
        ...
        builder.append(
                interpolateHtmlTableRowTemplate("NON-HEAP MEMORY USAGE", MEMORY_MX_BEAN.getNonHeapMemoryUsage()));
    ...
    private String buildJettyParametersHtmlTable(final String templateHtmlTable) throws Exception {
        ...
        final String endpointRegistration = HandlerUtils.linkifyRequestUrl(
                HttpScheme.HTTP.asString(), AdminPortalHandler.ADMIN_ROOT, host, adminPort);
    ...
    private String buildStubbyParametersHtmlTable(final String templateHtmlTable) throws Exception {
        ...
        builder.append(interpolateHtmlTableRowTemplate(
                "UPTIME", HandlerUtils.calculateStubbyUpTime(RUNTIME_MX_BEAN.getUptime())));
        builder.append(interpolateHtmlTableRowTemplate("INPUT ARGS", CommandLineInterpreter.PROVIDED_OPTIONS));
        builder.append(interpolateHtmlTableRowTemplate(
                "STUBBED ENDPOINTS", stubRepository.getStubs().size()));
        builder.append(interpolateHtmlTableRowTemplate(
                "LOADED YAML", buildLoadedFileMetadata(stubRepository.getYamlConfig())));
    ...
    private String buildLoadedFileMetadata(final File file) throws IOException {
        final StringBuilder builder = new StringBuilder();
        builder.append(String.format(TEMPLATE_LOADED_FILE_METADATA_PAIR, "parentDir", determineParentDir(file)))
                .append(HTML_BR);
        builder.append(String.format(TEMPLATE_LOADED_FILE_METADATA_PAIR, "name", file.getName()))
                .append(HTML_BR);
        builder.append(String.format(
                        TEMPLATE_LOADED_FILE_METADATA_PAIR,
                        "size",
                        String.format("%1$,.2f", ((double) file.length() / 1024)) + "kb"))
                .append(HTML_BR);
        builder.append(String.format(
                        TEMPLATE_LOADED_FILE_METADATA_PAIR,
                        "lastModified",
                        DateTimeUtils.systemDefault(file.lastModified())))
                .append(HTML_BR);

        return "<div style='margin-top: 5px; padding: 3px 7px 3px 7px; background-color: #fefefe'>" + builder.toString()
                + "</div>";
    }

    private String determineParentDir(final File file) throws IOException {
        return (ObjectUtils.isNull(file.getParentFile())
                ? file.getCanonicalPath().replaceAll(file.getName(), "")
                : file.getParentFile().getCanonicalPath() + "/");
    }

    private StringBuilder buildStubHtmlTableBody(
            final String resourceId, final String stubTypeName, final Map<String, String> stubObjectProperties)
            throws Exception {
        final StringBuilder builder = new StringBuilder();
    ...
    private String buildHtmlTableSingleRow(
            final String resourceId, final String stubTypeName, final String fieldName, final String value) {

        if (!ConfigurableYAMLProperty.isUnknownProperty(fieldName)
                && FIELDS_FOR_AJAX_LINKS.contains(ConfigurableYAMLProperty.valueOf(toUpper(fieldName)))) {
            final String cleansedStubTypeName =
                    stubTypeName.replaceAll(NEXT_IN_THE_QUEUE, ""); // Only when there are sequenced responses
            final String ajaxHyperlink =
                    String.format(TEMPLATE_AJAX_TO_RESOURCE_HYPERLINK, resourceId, cleansedStubTypeName, fieldName);
        ...
        if (fieldName.equals(ConfigurableYAMLProperty.URL.toString())) {
            final String urlAsHyperlink = HandlerUtils.linkifyRequestUrl(
                    HttpScheme.HTTP.asString(), escapedValue, jettyContext.getHost(), jettyContext.getStubsPort());
            final String tlsUrlAsHyperlink = HandlerUtils.linkifyRequestUrl(
                    HttpScheme.HTTPS.asString(), escapedValue, jettyContext.getHost(), jettyContext.getStubsTlsPort());

            final String tableRowWithUrl =
                    interpolateHtmlTableRowTemplate(StringUtils.toUpper(fieldName), urlAsHyperlink);
            final String tableRowWithTlsUrl =
                    interpolateHtmlTableRowTemplate("TLS " + StringUtils.toUpper(fieldName), tlsUrlAsHyperlink);
