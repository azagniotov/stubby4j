...
## User manual for stubby4j v7.0.1
### Table of contents
...
#### Gradle
```xml
compile("io.github.azagniotov:stubby4j:7.0.1")
```
or by adding a `classifier` to the JAR name like `no-dependencies` or `no-jetty`, i.e.:

```xml
compile("io.github.azagniotov:stubby4j:7.0.1:no-jetty")
```
...
```xml
compile("io.github.azagniotov:stubby4j:7.0.2-SNAPSHOT")
```
or by adding a `classifier` to the JAR name like `no-dependencie`s or `no-jetty`, i.e.:

```xml
compile("io.github.azagniotov:stubby4j:7.0.2-SNAPSHOT:no-jetty")
```
