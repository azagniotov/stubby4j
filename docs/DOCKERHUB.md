...
* [`11.0.0-jre16`](https://github.com/azagniotov/stubby4j/blob/v11.0.0/docker/jdk16/Dockerfile)
...
* [`11.0.0-jre11`](https://github.com/azagniotov/stubby4j/blob/v11.0.0/docker/jdk11/Dockerfile)
...
$ docker run --rm \
    ...
    -p 8882:8882 -p 8889:8889 -p 7443:7443 \
    azagniotov/stubby4j:11.0.0-jre11
```
...
* `11.0.0-jre11` is the tag specifying the `stubby4j` version. See the list above for relevant tags
...
services:
  stubby4j:
    image: azagniotov/stubby4j:11.0.0-jre11 # you can also use other tags: latest-jre11, latest-jre15
    volumes:
...
Currently, only in the Docker images tagged as `latest-jreXX` and `7.3.3-jreXX` and higher (i.e.: `11.0.0-jreXX`), the `stubby4j` service emits file-based logs (i.e.: generated by `log4j2` library) as well as `STDOUT` output stream logs. In images tagged as <= `7.3.2-jreXX`, only `STDOUT` output stream logs are available.
